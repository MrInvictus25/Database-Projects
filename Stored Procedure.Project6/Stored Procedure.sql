...
# SELECT count(TID) FROM aggressor_system.traducer;  # For depicting information what TIDs are deleted
SELECT * FROM aggressor_system.traducer;
SELECT * FROM aggressor_system.discriminator;
SELECT * FROM aggressor_system.resource;
            ...
            -- SET @Lim = @CountDuplicate - 1;
            -- SET @sql_data = CONCAT('DELETE A FROM aggressor_system.traducer A INNER JOIN (SELECT TID FROM aggressor_system.traducer 
            -- WHERE TID = "',TID_Dup,'" ORDER BY TID LIMIT ', @Lim, ' OFFSET 0);            

            -- SELECT @sql_data;
            -- PREPARE Del FROM @sql_data;
            -- EXECUTE Del;
